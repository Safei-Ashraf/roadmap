...
Progress and phase collapse state are stored in browser `localStorage`, so they are local to the browser/device. Start date and pace target (`startDate`, `targetPace`) live in the same UI state key for projected finish dates. A Pomodoro timer (work 15/30/45, breaks 5/15/30) lives in the status strip; last lengths and panel open state are saved, and session-end alarms play from `sounds/`. Completions are stamped with `completedAt` so conditional activity hints can show yesterday / this week hours and finished-phase vs ETA when those conditions are met. See `FULL_STACK_HANDOVER.md` for the MERN rebuild plan covering these features.
